...
para migrations/Livewire), e renderizar a árvore **Feature → Camada → arquivos** com
react-diff-view, camadas na ordem canônica e arquivos sem dono no balde **"Sem contexto"**
(por último). Navegação lateral feature → camada → arquivo.
...
- [ ] Cada arquivo renderiza seu diff via react-diff-view.
